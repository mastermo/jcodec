...
public class Cut {
    public static void main(String[] args) throws Exception {
        ...
        List<FileChannel> outs = new ArrayList<FileChannel>();
        try {
            input = new FileInputStream(source).getChannel();
            MovieBox movie = createRefMovie(input, "file://" + source.getCanonicalPath());
            List<MovieBox> slicesMovs;
            if (!selfContained) {
                out = new FileInputStream(new File(source.getParentFile(), removeExtension(source.getName())
                        + ".ref.mov")).getChannel();
    ...
    private static void saveSlices(List<MovieBox> slices, List<String> names, File parentFile) throws IOException {
        for (int i = 0; i < slices.size(); i++) {
            ...
            FileChannel out = null;
            try {
                out = new FileInputStream(new File(parentFile, names.get(i))).getChannel();
