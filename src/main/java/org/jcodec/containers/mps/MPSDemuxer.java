...
//                        map.put(pkt.pts, pkt.pos);
        ...
        public Packet getFrame(ByteBuffer buf) throws IOException {
...
//                buf.reset();
//            } else {
//                for (PacketizedES pes : streams.values()) {
//                    pes.es = null;
//                    pes.pending.clear();
//                }
//            }
...
//        }
        ...
        //
        // Entry<Long, Integer> before = map.floorEntry(pts);
        // if (before == null)
        // return -1;
        // Entry<Long, Integer> after = map.ceilingEntry(pts);
        //
        // int toSeek;
        // if (pts - before.getKey() < 2 * TIMESCALE) {
        // toSeek = before.getValue();
        // } else if (after == null) {
        // int bOff = before != null ? before.getValue() : 0;
        // toSeek = (bOff + buf.limit()) / 2;
        // } else {
        // int bOff = before.getValue();
        // int aOff = after.getValue();
        // long bTv = before.getKey();
        // long atv = after.getKey();
        // long ts = atv - pts < TIMESCALE ? max(0, atv - TIMESCALE) : pts;
        //
        // toSeek = (int) (bOff + (aOff - bOff) * (ts - bTv) / (atv - bTv));
        // }
        //
        // buf.position(toSeek);
        // PESPacket pkt;
        ...
        // if (MPEGUtil.gotoMarker(buf, 0, PRIVATE_1, VIDEO_MAX) == null)
        // return null;
        //
        // return readPES(buf, buf.position());
        // }
    }
